...
export default function MissionControl({
  ...
  return (
    <section id="mission" className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm shadow-indigo-100/30 scroll-mt-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-100 gap-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-lg font-bold text-slate-800 tracking-tight">Mission Control</h2>
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg text-xs gap-1">
          <button 
            ...
            className={`px-3 py-1.5 rounded-md font-semibold transition-all cursor-pointer
              ${activeTab === 'live' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
          >
            Live Mission
          </button>
          <button 
            ...
            className={`px-3 py-1.5 rounded-md font-semibold transition-all cursor-pointer
              ${activeTab === 'portal' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
          >
            Portal Status
          </button>
          <button 
            ...
            className={`px-3 py-1.5 rounded-md font-semibold transition-all cursor-pointer
              ${activeTab === 'activity' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-600 hover:text-slate-800'}`}
          >
            Activity Log
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pt-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono font-bold text-slate-800">UCID-A</span>
                  <span className="flex items-center gap-1.5 text-[10px] text-indigo-755 font-bold bg-indigo-50 px-2 py-0.5 rounded-full">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-sm font-bold text-slate-700">{isStopped ? 'Stopped' : '60%'}</span>
                    <div className="flex-1 bg-slate-205 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-indigo-600 h-full rounded-full" style={{ width: isStopped ? '0%' : '60%' }} />
                    </div>
                  </div>
                </div>
                <div className="text-[10px] font-semibold text-slate-500">Sequential (2 of 3 configs)</div>
              </div>

              {/* UCID-B Progress Item with Auto-Healed Event */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 space-y-3 relative">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono font-bold text-slate-800">UCID-B</span>
                  <span className="flex items-center gap-1.5 text-[10px] text-emerald-605 font-bold bg-emerald-50/70 px-2 py-0.5 rounded-full">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-sm font-bold text-slate-700">{isStopped ? 'Stopped' : '48%'}</span>
                    <div className="flex-1 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-indigo-600 h-full rounded-full" style={{ width: isStopped ? '0%' : '48%' }} />
                    </div>
                  </div>
                </div>
                <div className="text-[9px] font-bold text-amber-700 bg-amber-50 rounded px-1.5 py-0.5 flex items-center gap-1">
                  <AlertTriangle className="w-2.5 h-2.5" /> Auto-healed (resolved connection block)
                </div>
              </div>

              {/* UCID-C Progress Item (Queued) */}
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3.5 space-y-3 opacity-80">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-mono font-bold text-slate-800">UCID-C</span>
                  <span className="flex items-center gap-1.5 text-[10px] text-amber-700 font-bold bg-amber-50 px-2 py-0.5 rounded-full">
                    Queued
                  </span>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-sm font-bold text-slate-400">31%</span>
                    <div className="flex-1 bg-slate-200 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-slate-300 h-full rounded-full" style={{ width: '31%' }} />
                    </div>
                  </div>
                </div>
                <div className="text-[10px] font-semibold text-slate-400">Queued — waiting for browser slot</div>
              </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1 font-medium"><strong>{processedParts}</strong> of 412 core configuration parts parsed</p>
              </div>
                    ...
                    <div key={idx} className="flex flex-col items-center">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all
                        ${stepIsComplete 
                          ...
                          : stepIsActive 
                            ? 'bg-indigo-50 text-indigo-650 border border-indigo-405 shadow-md animate-pulse' 
                            : 'bg-white border text-slate-400'}`}>
                        {stepIsComplete ? '✓' : idx + 1}
                      </div>
                      <span className="text-[10px] font-semibold text-slate-650 mt-2 truncate max-w-full">
                        {step.label}
                      </span>
                    </div>
                  );
            ...
            <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-200/50">
              <button
                ...
                disabled={isStopped}
                className="px-4 py-2 bg-white hover:bg-slate-50 border rounded-lg text-xs font-semibold text-slate-700 inline-flex items-center gap-1.5 transition shadow-sm disabled:opacity-50 cursor-pointer"
              >
                {isPause ? (
                  <>
                    <Play className="w-3.5 h-3.5 text-emerald-500 fill-emerald-500" /> Resume Mission
                  </>
                ) : (
                  <>
              ...
              <button
                ...
                className="px-4 py-2 bg-white hover:bg-slate-50 border rounded-lg text-xs font-semibold text-slate-700 inline-flex items-center gap-1.5 transition shadow-sm cursor-pointer"
              >
              ...
              {isStopped ? (
                <button
                  ...
                  onClick={handleRestartMission}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold inline-flex items-center gap-1.5 transition shadow cursor-pointer animate-pulse"
                >
        ...
        <div className="space-y-5">
              ...
              {missionSteps.map((step) => (
                <div key={step.id} className="flex items-center justify-between text-xs py-0.5">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="w-4 h-4 rounded bg-slate-200/80 text-slate-600 flex items-center justify-center text-[9px] font-mono font-bold">
                      {step.id}
                    </span>
                    <span className={`truncate ${step.status === 'Completed' ? 'text-slate-500 line-through' : 'text-slate-800 font-medium'}`}>
                      {step.name}
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {step.status === 'Completed' ? (
                      <span className="text-[10px] font-mono text-emerald-600 font-bold flex items-center gap-1 bg-emerald-50 px-1.5 py-0.2 rounded">
                        ✓ {step.time}
                      </span>
                    ) : step.status === 'In Progress' && !isStopped ? (
                      <span className="text-[10px] font-mono text-indigo-700 font-bold flex items-center gap-1.5 bg-indigo-50 px-1.5 py-0.2 rounded animate-pulse">
                        <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full animate-ping" /> {step.processed}
                      </span>
                    ) : (
                      <span className="text-[10px] text-slate-400 font-medium bg-slate-100 px-1.5 rounded">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4">
            ...
            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-50/50">
              ...
              <div className="grid grid-cols-2 gap-2">
                {portalConnections.map((conn) => (
                  <div key={conn.name} className="flex items-center justify-between p-2 rounded-lg bg-white border border-slate-100 text-xs">
                    <span className="font-semibold text-slate-700">{conn.name}</span>
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded
                      ${conn.status === 'Connected' ? 'bg-emerald-50 text-emerald-700' : conn.status === 'Delayed' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                      {conn.status}
                    </span>
                  </div>
                ))}
            ...
            <div className="border border-slate-200 rounded-2xl p-4 bg-slate-900 text-slate-200 space-y-3 relative overflow-hidden group">
              <div className="flex items-center justify-between">
              ...
              <div className="h-28 bg-slate-950 rounded-lg border border-slate-800 flex flex-col items-center justify-center p-3 text-center relative overflow-hidden">
                  ...
                  <>
                    ...
                    <div className="w-full h-full flex flex-col justify-between text-left font-mono text-[8px] text-emerald-400 leading-snug z-10 select-none pb-0.5">
                      ...
                      <div className="space-y-0.5 py-1">
                        ...
                        <div className="text-emerald-500">&gt; OK: Loaded 3 server chassis maps successfully.</div>
                      </div>
                      <div className="flex justify-between text-slate-500 border-t border-slate-900 pt-1 text-[7px]">
